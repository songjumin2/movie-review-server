...
exports.getMovies = async (req, res, next) => {
  ...
  let query = `select if(f.user_id is null, 0, 1) as is_favorite, m.*, count(r.movie_id) as reply_cnt, round(avg(r.rating), 1) as avg_rating  
    from mytable as m
...
exports.getAuthMovies = async (req, res, next) => {
  ...
  // ${offset}, ${limit} 대신 물음표로 해도됨
  let query = `select m.*, 
      if(f.id is not null, true, false) as is_favorite, count(r.movie_id) as reply_cnt, round(avg(r.rating), 1) as avg_rating  
      from mytable as m
      left join movies_reply as r
      on m.id = r.movie_id
      left join movies_favorite as f
