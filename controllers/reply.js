...
// @desc     해당 영화의 나의 댓글만 가져오는 API
// @route    GET  /api/v1/reply/myReview?movie_id=124&offset=0&limit=25&order=desc
// @request  movie_id, offset, limit
// @response  sucess, items:[], cnt
// 물음표 뒤에있는건 쿼리에서 찾아야함
exports.myGetReply = async (req, res, next) => {
  let movie_id = req.query.movie_id;
  let user_id = req.user.id;
  let offset = req.query.offset;
  let limit = req.query.limit;
  ...
  join mytable as m on r.movie_id = m.id 
  join movie_review_user as u  on r.user_id = u.id 
  where r.movie_id = ${movie_id} and u.id = ${user_id} order by r.created_at ${order} limit ${offset}, ${limit} `;
  // offset, limit 는 넘버 붙여줘야함
  let data = [movie_id, user_id, order, Number(offset), Number(limit)];
  try {
    [rows] = await connection.query(query, data);
